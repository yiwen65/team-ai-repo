...
description: >
  ...
  EM Planner、Lattice Planner、OpenSpace Planner、MPC Controller、LQR Controller。
  涉及场景管理器（ScenarioManager）、任务调度（TaskFactory）、Deciders（Path/Speed/ST Bounds）、
  Optimizers（PiecewiseJerk、DP/Quadratic Programming）、安全验证（RSS）。
  在以下场景触发使用：
...
│   ├── lat_controller.cc     # 横向控制器 (LQR)
│   └── lon_controller.cc     # 纵向控制器 (PID)
├── common/                   # 公共工具
│   ├── interpolation_1d.cc
│   ├── interpolation_2d.cc
